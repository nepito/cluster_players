...
metrics <- c("second", "just_select_daves", "soccerment", "central_attackers", "central_midfielder")
...
sorted_players <- tibble::rowid_to_column(sorted_players, "ID")
for (macro_grupo in 1:5) {
  ...
  lp <- macro_names |>
    pivot_longer(!c("ID", "year"), names_to = "variable", values_to = "deciles")
